...
function Dzielenie() {
    ...
    if (Liczba2 == 0) {
        //Jeżeli dzielnik jest równy 0, wtedy:
...
function Pierwiastkowanie(liczba) {
    ...
    if (stopien == 1) {
        alert(
            "Pierwiastek 1 stopnia jest taki sam jak liczba, którą chce się spierwiastkować!"
        );
        document.getElementById(
            "Dzialanie"
        ).innerHTML = `&#8730${liczba} = ${liczba}`;
        return liczba;
    } else if (stopien == 2) {
        ...
        document.getElementById("Dzialanie").innerHTML =
            "Nie jest zaimplementowane stopniowanie pierwiastków powyżej stopnia 3!";
        return null;
    }
}
...
function Potegowanie(liczba, wykladnik) {
    ...
    document.getElementById(
        "Dzialanie"
    ).innerHTML = `${liczba}^${wykladnik} = ${Wynik}`;
}
